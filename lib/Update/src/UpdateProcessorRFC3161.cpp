...
int UpdateProcessorRFC3161::addTrustedCerts(const unsigned char * der, size_t len) {
  int ret;
...
#if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_DEBUG
  {
    ...
    for (const mbedtls_x509_crt * c = _reply.chain; c; c = c->next) {
      ...
      if (mbedtls_x509_crt_verify(_reply.chain, _trustChain, NULL /* no CRL */, NULL /* any name fine */, &results, NULL, NULL) != 0) {
        ...
        log_e("RFC3161 signature could not be validated against the chain (%#06x): %s. Aborting.", results, err_mess);
        return secure_update_processor_ERROR;
      }
