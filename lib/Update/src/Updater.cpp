...
static bool _partitionIsBootable(const esp_partition_t* partition) {
  ...
  if (!ESP.partitionRead(partition, 0, (uint32_t*)buf, 4)) {
...
static bool _enablePartition(const esp_partition_t* partition) {
  ...
  // if (!ESP.partitionWrite(partition, 0, (uint32_t*) _skipBuffer, ENCRYPTED_BLOCK_SIZE))
  if (!ESP.partitionWrite(partition, 0, (uint32_t*)buf, 4)) {
...
UpdateClassEx::UpdateClassEx(UpdateProcessor  * processor)
  : _processor(0)
  , _error(0)
  , _buffer(0)
  , _bufferLen(0)
...
UpdateClassEx::~UpdateClassEx() {
  _reset();
  if (_processor)
    delete(_processor);
}

void UpdateClassEx::setProcessor(UpdateProcessor * processor) {
  _reset();
  if (_processor)
    delete(_processor);
...
UpdateClassEx& UpdateClassEx::onProgress(THandlerFunction_Progress fn) {
  _progress_callback = fn;
  return *this;
}

void UpdateClassEx::_reset() {
  if (_buffer)
    delete[] _buffer;
  _buffer = 0;
  _bufferLen = 0;
...
bool UpdateClassEx::canRollBack() {
...
bool UpdateClassEx::begin(size_t size, int command, int ledPin, uint8_t ledOn) {
  if (_size > 0) {
...
void UpdateClassEx::_abort(uint8_t err) {
  _reset();
  _error = err;
}

void UpdateClassEx::abort() {
  log_d("Aborted.");
  _abort(UPDATE_ERROR_ABORT);
}

bool UpdateClassEx::_writeBuffer() {
  ...
  if (!_post_header) {
    ...
    if (e != UpdateProcessor::secure_update_processor_OK) {
      log_e("Error during processing of process_header.");
  ...
  // log_d("_bufferLen: %d, size=%d, progress=%d, remaining=%d", _bufferLen, _size, _progress, remaining());
  /* Check is not valid if we are streaming from an http upload where we don't know the exact fw image size and only set the max available partition size!
  if (_bufferLen != SPI_FLASH_SEC_SIZE && remaining() != _bufferLen) {
    log_e("Buffer passed is too short (and not the last one) - should never happen! _bufferLen: %d, size=%d, progress=%d, remaining=%d", _bufferLen, _size, _progress, remaining());
    // No risk no fun
  ...
  */
  ...
  if (!ESP.partitionEraseRange(_partition, _progress_flash, SPI_FLASH_SEC_SIZE)){
  ...
  if (!ESP.partitionWrite(_partition, _progress_flash, (uint32_t*)_buffer, _bufferLen)) {
...
bool UpdateClassEx::end(bool evenIfRemaining) {
...
size_t UpdateClassEx::write(uint8_t *data, size_t len) {
  size_t written = 0;
...
template<typename T>
size_t UpdateClassEx::write(T & data) {
  size_t written = 0;
  if (hasError() || !isRunning())
    return 0;
...
size_t UpdateClassEx::writeStream(Stream &data) {
    size_t written = 0;
    size_t toRead = 0;
...
void UpdateClassEx::printError(Stream & out) {
  out.println(_err2str(_error));
}

const char * UpdateClassEx::errorString() {
  return _err2str(_error);
}

UpdateClassEx UpdateEx;
