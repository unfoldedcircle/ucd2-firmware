// SPDX-FileCopyrightText: Copyright (c) 2022 Unfolded Circle ApS and/or its affiliates <hello@unfoldedcircle.com>
// SPDX-License-Identifier: GPL-2.0-or-later
//
// Revision 3
// UCD2 prototype, 2 round holes to the middle, last revision with 1 external IR output
//
#pragma once

#define CHARGE_SENSE_GPIO    39
#define BUTTON_GPIO          0

#define IR_RECEIVE_PIN       22
#define IR_SEND_PIN_INT_SIDE 19
#define IR_SEND_PIN_EXT_1    18

#define STATUS_LED_R_PIN     25
#define STATUS_LED_G_PIN     26
#define STATUS_LED_B_PIN     27
